...
# The wrapper at /usr/local/bin/claude (installed by the .txz) finds and execs
# whatever lives under /root/.local/share/claude/versions/. Do NOT overwrite
# /usr/local/bin/claude here — that's the wrapper.
if [ -f "${BIN_CACHE}" ]; then
...
# PATH: put /usr/local/bin (the wrapper) before /root/.local/bin so user
# invocations go through the wrapper even if Claude's installer rewrites
# the /root/.local/bin/claude symlink.
